...
function addoneline() {
	i++;
	var oneline = "<div style='width:50%' name='"+i+"'><select name='photo"
			+ i
			+ "type' class='input w50'><option value=''>请选择图片</option><option value='indoor'>室内图</option><option value='show'>展示图</option><option value='tile'>平铺图</option><option value='envir'>环境图</option></select></div><div name='"+i+"'><a name='"+i+"' style='float: left;margin-top: 10px;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a><input type='file'  accept='image/*' class='input w50' value='' style='width:12%;' name='photo"
			+ i
			+ "' /></div><div name='"+i+"'><a name='"+i+"' style='float: left;margin-top: 10px;'>&nbsp;&nbsp;&nbsp;&nbsp;备注：</a><input type='text' class='input w50' name='photo"
			+ i
			+ "remark' style='width:18%;float:left;' /></div><input type='hidden' name='projectid'/><input type='button' name='"+i+"' value='移除' onclick='removealine(this)'/><br><br><br>";
...
function removealine(e){
	var removebtnname=e.name;
	document.getElementsByName("photo"+removebtnname)[0].remove();
	document.getElementsByName("photo"+removebtnname+"type")[0].remove();
	document.getElementsByName("photo"+removebtnname+"remark")[0].remove();
	var hh=document.getElementsByName(removebtnname);
	for(var w=0;w<hh.length+3;w++){
		hh[0].remove();
	}
}
function addoperate() {
...
function finddetail(){
		...
		window.location="/NfCrmManager/proj/rebackdata.do?pro_id="+parent_type;
	}
/** +++++++++++++++++++++++++++++++++结束++++++++++++++++++++++++++ */
...
$(document).ready(
		function() {
			$.ajax({
				type : "GET",
				url : "/NfCrmManager/template/selectTemplate2.do",
				async : true,
				dataType : "json",
		...
		/*	查公司项目*/
			$.ajax({
				type : "POST",
				url : "/NfCrmManager/proj/queryprotype.do",
				async : true,
				dataType : "json",
				success : function(ptype) {
					var protype = ptype.protypelist;
					var options= null;
					var proid=$("input[name='findproid']").val();
					for (var i = 0; i < protype.length; i++) {
						if(protype[i].pro_id==proid){
							options = "<option selected='selected' value='" + protype[i].pro_id + "'>"
							+ protype[i].pro_name + "</option>";
						}else{
							options = "<option value='" + protype[i].pro_id + "'>"
							+ protype[i].pro_name + "</option>";
						}
...
function addsubbtn() {
		...
		var findproid=$("input[name='findproid']").val();
		var tag="null";
		for(var h=0;h<i;h++){
				var type=$("select[name='photo"+i+"type']").val();
				var remark=$("input[name='photo"+i+"remark']").val();
				var photo=$("input[name='photo"+i+"']").val();
				if(type==""||remark==""||photo==""){
					tag+="not";
				}
			}
		if(areamin==""||areamax==""||pro_name==""||pricemin==""||pricemax==""||pro_type==""||pro_template==""||spread_type==""||region==""||actual_price==""||actual_area==""||
				p_type==""||profession==""||features==""||address==""||floor==""||property_price==""||decoration==""||food==""||phone==""||tel==""){
			swal({title: "温馨提醒",text: "项目基本信息未填写完整！",type: "warning",showCancelButton: false,confirmButtonColor: "#DD6B55",confirmButtonText: "确定"});
    			
		}else if(tag!="null"){
			swal({title: "温馨提醒",text: "项目图片信息未填写完整！",type: "warning",showCancelButton: false,confirmButtonColor: "#DD6B55",confirmButtonText: "确定"});
		}else if(str==""){
			swal({title: "温馨提醒",text: "环境设施信息未填写完整！",type: "warning",showCancelButton: false,confirmButtonColor: "#DD6B55",confirmButtonText: "确定"});
		}else{
		var date = {
			confi : str,
			pro_name :pro_name,
	...
	$.ajax({
		type : "POST",
		url : "/NfCrmManager/proj/addproMsg.do",
		data : date,
		async : true,
		dataType : "json",
		success : function(rsp) {
			$("input[name='projectid']").val(rsp.pro_id);
				...
				var url = "/NfCrmManager/proj/upload.do";
				$.ajax({
					url : url,
					type : 'POST',
					data : formData,
					...
					success : function(responseStr) {
		        		...
		        		function(){
		        			window.location = "/NfCrmManager/proj/selectallpro.do";
		        		});
					},
					error : function(responseStr) {
						swal({title: "系统提示",text: "新增项目失败！",type: "warning",showCancelButton: false,confirmButtonColor: "#DD6B55",confirmButtonText: "确定"});
					}
				});
		},
		error : function(rec) {
